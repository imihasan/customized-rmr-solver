...
%                     considered or not
%
% * execlude_locked: true or false whether locked coordinates are to be
%                    execluded
%
% * execl_clav: true or false whether coordinates of the clavicle are to be
%               execluded
%
% * execl_wing: true or false whether scapula winging coordinate is to be
%               execluded
%
%
...
%
% * tag_cost: string to define which penalty to use: "conditional",
%            "planar", "curve" or "no_penalty"
%
% OUTPUT:
...
%% Deactivating AC Constraint
model_temp.updConstraintSet().get("AC").set_isEnforced(1);
...
%% Lock GH translations before inverse kinematics
% model_temp.updCoordinateSet().get("GH_Tx").set_locked(1);
% model_temp.updCoordinateSet().get("GH_Ty").set_locked(1);
% model_temp.updCoordinateSet().get("GH_Tz").set_locked(1);
...
if trc_file
    ...
    ikSetupFile = [path_to_repo,'' ...
            '\ExperimentalData\IK setup files\IKSetup_Orthoload.xml'];
    ...
    %Comment out the below if scapula is to be trackedbased on the
    %experimental mrkers
    ...
    ikTool.print(fullfile(saving_path,append(experiment_name,'RMR_autogenerated_IK_setup.xml')));
...
model_temp.print(fullfile(saving_path,append(experiment_name,'_Model.osim')));

%% Unlock GH translations after inverse kinematics
% model_temp.updCoordinateSet().get("GH_Tx").set_locked(0);
% model_temp.updCoordinateSet().get("GH_Ty").set_locked(0);
% model_temp.updCoordinateSet().get("GH_Tz").set_locked(0);
...
%Here we choose to remove the clavicle coordinates because they are
%redundant coordinates
if execl_clav==1
    ex_Names=["clav_prot", "clav_elev"];
...
%Execlude winging coordinate because it's a redundant coordinate
if execl_wing==1
    ex_Names=["scapula_winging"];
    indx1=find(coordNames==ex_Names(1));
    coordinates(:,indx1)=[];
    coordNames(indx1)=[];
end
...
trans_coord=["GH_Tx", "GH_Ty", "GH_Tz","ground_thorax_tx", "ground_thorax_ty",  "ground_thorax_tz"];
...
%Execlude clavicle coordinate actuators
if execl_clav==1
    ex_Names=["clav_prot", "clav_elev"];
...
epsilon = 0;
joints_penalized=["GlenoHumeral","scapulothoracic"]; %Joints whose reserve actuators will be penalizedby assiging a weigth of 10
wc=ones(1,numCoordActs);
...
for time_instant = 1:numTimePoints
    ...
    for j = 1:length(All_coordNames)
        ...
        coord.setValue(state,All_coordinates(time_instant,j), false); % instead of false replace so that does the assembly on the last call (j==length)
    ...
    [~, Vec_H2GC,Vec_H2GE,Vec_GC2GE,G_Cent_Loc(time_instant,:)] = get_glenoid_status(model_temp, state); 
    ...
    if flag_GH_enforced
        ...
        if exitflag<0 && time_instant>1
            ...
            [x,~,exitflag,output] = fmincon(@(x)cost(x, w, Vec_H2GC, Vec_GC2GE, A_eq_force, F_r0, tag_cost), xsol(time_instant-1, :), [], [], A_eq_acc, Beq, lb, ub, @(x)jntrxncon_linForce(x, Vec_H2GC, Vec_GC2GE, A_eq_force, F_r0, tag_con), options);
        end
    else
        [x,~,exitflag,output] = fmincon(@(x)cost(x, w, Vec_H2GC, Vec_GC2GE, A_eq_force, F_r0, tag_cost), x0, [], [], A_eq_acc, Beq, lb, ub, [], options);
        if exitflag ==0
            % call the solver again, starting from current x, in case the maximum iterations are exceeded
            [x,~,exitflag,output] = fmincon(@(x)cost(x, w, Vec_H2GC, Vec_GC2GE, A_eq_force, F_r0, tag_cost), x, [], [], A_eq_acc, Beq, lb, ub, [], options);
        end
        if exitflag<0 && time_instant>1
            % call the solver again, starting from previous optimum found,
            % in case optimization gets stuck in local minimum 
            [x,~,exitflag,output] = fmincon(@(x)cost(x, w, Vec_H2GC, Vec_GC2GE, A_eq_force, F_r0, tag_cost), xsol(time_instant-1, :), [], [], A_eq_acc, Beq, lb, ub, [], options);
        end
    end
    ...
    if print_flag
        ...
        force_vector(time_instant,:)=force_vec;
        ...
        %The below method for finding the rotation matrix gets stuck in
        %gimbal lock
        ...
        %The below method is alternatively implemented to find the rotation
        %matrix
        xhat=-norm_Vec_H2GC;
        ...
        R=[xhat; yhat; zhat];
        norm_fv_rotated(time_instant,:) = R*norm_fv_in_ground(time_instant,:)'*(norm(1000*Vec_H2GC)/cosTheta);

        %Store variables 
        Vec_GC2GEE(time_instant,:)=Vec_GC2GE;
        Vec_H2GCC(time_instant,:)=Vec_H2GC;
        norm_Vec_H2GE(time_instant,:)=Vec_H2GE/norm(Vec_H2GE);
        norm_Vec_H2GCC(time_instant,:)=norm_Vec_H2GC;
...
if print_flag
    ...
    saveas(f1, name_fig1)
    name_fig1 = append(experiment_name, '_MuscleActivations.svg');
    saveas(f1, name_fig1)
    ...
    plot(p, 'FaceColor', 'r')
    for time_instant=1:numTimePoints
        scatter(-norm_fv_rotated(time_instant,2), -norm_fv_rotated(time_instant,3), [], c(time_instant), 'filled')
    end
    hcb = colorbar;
    h = gca;
    axis equal
    ...
    quiver(-norm_fv_rotated(1:10:end,2),-norm_fv_rotated(1:10:end,3),-norm_fv_rotated(1:10:end,2), ...
    -norm_fv_rotated(1:10:end,3),"filled","AutoScale","on","Alignment","head", ...
...
save(name_file, 'xsol', 'muscle_order', 'frequency_solution', 'optimizationStatus', 'unfeasibility_flags', 'tOptim',"simulatedAccelerations", ...
    'force_vector','norm_fv_in_ground','norm_fv_rotated','All_coordNames',"All_coordinates","norm_Vec_H2GCC","norm_Vec_H2GE","Vec_H2GCC","Vec_GC2GEE","G_Cent_Loc", ...
    "R");
